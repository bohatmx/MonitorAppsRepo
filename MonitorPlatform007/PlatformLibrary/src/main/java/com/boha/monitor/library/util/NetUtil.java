...
public class NetUtil {
    ...
    public static void sendRequest( Context ctx,  RequestDTO request,  NetUtilListener utilListener) {
        ...
        Log.d("NetUtil","########### sendRequest ... getRideWebSocket: " + request.isRideWebSocket());
    ...
    public static void sendRequest( Context ctx,  RequestList request,  NetUtilListener utilListener) {
        listener = utilListener;
        Log.d("NetUtil","########### sendRequest ... getRideWebSocket: " + request.isRideWebSocket());
        WebCheckResult wcr = WebCheck.checkNetworkAvailability(ctx, true);
        if (!wcr.isMobileConnected() && !wcr.isWifiConnected()) {
        ...
        if (request.isRideWebSocket()) {
            sendListViaWebSocket(ctx,request);
        } else {
            sendListViaHttp(ctx,request);
        }
    }
    ...
    private static void sendViaHttp(Context ctx, RequestDTO request) {
        BaseVolley.getRemoteData(Statics.GATEWAY_SERVLET, request, ctx, new BaseVolley.BohaVolleyListener() {
            ...
            public void onVolleyError(VolleyError error) {
                Log.e("NetUtil", "-- Volley Error: " + error.getMessage());
                listener.onError("Error communicating with server");
            }
        });
    }
    private static void sendListViaWebSocket(Context ctx, RequestList request) {
    ...
    private static void sendListViaHttp(Context ctx, RequestList request) {
        BaseVolley.getRemoteData(Statics.GATEWAY_SERVLET, request, ctx, new BaseVolley.BohaVolleyListener() {
