...
/**
 * Created by aubreyM on 15/01/22.
 */
...
public class UnhandledExceptionHandler
    ...
    private static final String LOG = UnhandledExceptionHandler.class.getSimpleName();
    private final Activity activity;


    public UnhandledExceptionHandler(final Activity activity) {
        this.activity = activity;
        Log.w(LOG,"## UncaughtExceptionHandler constructor done.");
    }

    public void uncaughtException(Thread unusedThread,  final Throwable e) {
        activity.runOnUiThread(new Runnable() {
            @Override public void run() {
                Log.e(LOG,"## uncaughtException has fired ...");
